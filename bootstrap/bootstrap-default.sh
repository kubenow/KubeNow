...
if [[ "$node_labels" == *"role=master"* ]]; then
  ...
  if [ -n "$API_ADVERTISE_ADDRESSES" ]; then
    ...
    kubeadm init --token "${kubeadm_token}" --token-ttl=0 --pod-network-cidr=10.244.0.0/16 --kubernetes-version=v1.10.7 --ignore-preflight-errors=cri --api-advertise-address="$API_ADVERTISE_ADDRESSES"
  else
    # shellcheck disable=SC2154
